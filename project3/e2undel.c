...
void explore_inode(int local_inode_index, int inode_table_block, int inode_size, int fd, int block_size, int first_data_block, int inode_number, int active);
void explore_inodes(int fd, struct ext2_group_desc * group_desc, int inodes, int block_size, int first_data_block, int first_inode, int inode_table_block, int inode_size, int block_group); 
...
int main (int argc, char* argv[]) {
  ...
  for (i = 0; i < block_groups; i++) {
    ...
    //  first_block_flag = 0;
    //}
    explore_inodes(fd, &group_description, super_block.s_inodes_per_group, block_size, super_block.s_first_data_block, first_inode_index, group_description.bg_inode_table, super_block.s_inode_size, i);
  }
...
void explore_inodes(int fd, struct ext2_group_desc * group_desc, int inodes, int block_size, int first_data_block, int first_inode, int inode_table_block, int inode_size, int block_group) {
  ...
  char * bitmap = (char *) malloc(inodes / 8);
  assert(bitmap != NULL);
  printf("Examining an inode bitmap\n"); 
  
  // Copy the bitmap into main memory.
  assert(read(fd, bitmap, inodes / 8) == inodes / 8);

  // Loop over each bit in the bitmap, if 0 we have a 'free' inode to examine. 
  for (i = first_inode; i < inodes; i++) {
    char byte = bitmap[i / 8];
    int bit = (byte >> i % 8) & 1; 
    //if (bit != 1) {
      // These are 'active' inodes that we won't end up worrying about.
      // printf("bit %d = %d\n", i, bit);
      // In the 'easy' file system, I assumed there would be none of these, but
      // one occurs on bit 12. Let's explore that inode.
      explore_inode(i, inode_table_block, inode_size, fd, block_size, first_data_block, 1 + (block_group * inodes) + i, bit);  
   // }
  }
  free(bitmap);
}
...
void explore_inode(int local_inode_index, int inode_table_block, int inode_size, int fd, int block_size, int first_data_block, int inode_number, int active) {
  ...
  // Need to grab the inode. First seek to the apporpriate block.
  int inode_byte_offset = inode_size * local_inode_index;
  int block;
  if (inode_byte_offset == 0) {
    block = inode_table_block;  // Avoid divide by zero error.
  } else {
    block = inode_table_block + (block_size / inode_byte_offset);
  }
  lseek(fd, getByteOffset(block, first_data_block, block_size), SEEK_SET);  
 
  // Read the inode.
  read(fd, &inode, sizeof(inode));
  //if (inode.i_size != 0) {  // We may have something interesting. 
    // Print:
    printf("The following node is active: %d  ", active);
